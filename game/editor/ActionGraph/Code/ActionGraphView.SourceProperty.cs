...
partial class ActionGraphView
{
	...
	private static SerializedProperty? ResolveProperty(object? hostObject, ActionGraph? graph, IActionGraphDelegate? agDelegate)
	{
		...
		if (hostObject is IBlowoutValidable { IsValid: false }) return null;
		if (hostObject is Component comp && comp is IValid { IsValid: false }) return null;
			...
			case GameObject go:
				{
					...
					{
						if (cmp is Component compf)
						{
							if (FindProperty(compf, graph, agDelegate) is { } prop)
							{
								return prop;
							}
						}
						else if (cmp is IBlowoutValidable validable)
						{
							if (FindProperty(validable, graph, agDelegate) is { } prop)
							{
								return prop;
							}
						}
	...
	private static bool IsPropertyValueSerialized(object inst, SerializedProperty property)
	{
		...
		if (property.PropertyType.IsAssignableTo(typeof(IBlowoutSerializable))) return false;
		...
		if (property.HasAttribute<BlowoutExposeField>()) return true;
	...
	/// </summary>
	private static SerializedProperty? FindProperty(IBlowoutValidable root, ActionGraph? graph, IActionGraphDelegate? agDelegate)
	{
		if (graph is null && agDelegate is null) return null;
	...
	private static bool AppendProperty(TextWriter writer, SerializedProperty property)
	{
		...
		while (target is not null)
		{
			switch (target)
			{
				case IBlowoutGameSystem sys:
					writer.WriteLine($"    in {sys} ({RuntimeHelpers.GetHashCode(sys):x8})");
					target = (GameObject)sys.SystemGameObject;
					break;
